...
class AQIAnalyzer:
    ...
    def _format_url(self, country: str, state: str, city: str) -> str:
        ...
        return (
            f"https://www.aqi.in/dashboard/{country_clean}/{state_clean}/{city_clean}"
        )
...
class HealthRecommendationAgent:
    ...
    def __init__(self, openai_key: str) -> None:
        self.agent = Agent(
            model=OpenAIChat(
                id="gpt-4o", name="Health Recommendation Agent", api_key=openai_key
            )
        )
...
def analyze_conditions(user_input: UserInput, api_keys: Dict[str, str]) -> str:
    ...
    aqi_data = aqi_analyzer.fetch_aqi_data(
        city=user_input.city, state=user_input.state, country=user_input.country
    )
...
def render_sidebar():
    ...
    with st.sidebar:
        ...
        if (
            ...
            and new_openai_key
            and (
                new_firecrawl_key != st.session_state.api_keys["firecrawl"]
                or new_openai_key != st.session_state.api_keys["openai"]
            )
        ):
            st.session_state.api_keys.update(
                {"firecrawl": new_firecrawl_key, "openai": new_openai_key}
            )
...
def render_main_content():
    ...
    with col1:
        ...
        state = st.text_input(
            "State",
            placeholder="If it's a Union Territory or a city in the US, leave it blank",
        )
...
def main():
    ...
    if st.button("🔍 Analyze & Get Recommendations"):
        if not all([user_input.city, user_input.planned_activity]):
            st.error(
                "Please fill in all required fields (state and medical conditions are optional)"
            )
